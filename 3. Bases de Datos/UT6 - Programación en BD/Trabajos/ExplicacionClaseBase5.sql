...
begin
  ...
  declare cur1 cursor for select DNI, Nombre from Socios;

  -- Declaración del manejador de error
  -- Si el cursor llega a su fin el valor de la variable fin 
  -- pasará a ser TRUE
  ...
  -- Es decir, para recorrer la lista de números y nombres de socios
  while (not fin) do
	fetch cur1 into vDNI,vNombre;
    select vDNI,vNombre;
  end while;
...
-- el nombre y los 2 apellidos de los socios que han alquilado alñ menos 2 libros
-- podemos usar los punteros porque no hay bucle
...
-- libros que ha alquilado un tio en concreto
drop procedure if exists P01;
DELIMITER $$
create procedure P01()
begin
...
DELIMITER $$
create procedure P06(in DNIExt varchar(9))
begin
	declare vDNI varchar(9);
	declare vNombre varchar(20);
    declare vNombre varchar(20);
    ...
    open cur6;
    while (not fin) do
		fetch cur6 into vDNI, vNombre, vApellido;
        set salida = concat(salida, vDNI, " ", vNombre, " ", vApellido1, "\n");
    end while;
