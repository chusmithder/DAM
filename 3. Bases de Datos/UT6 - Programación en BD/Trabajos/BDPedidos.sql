...
DELIMITER $$
create procedure actualizarPrecio(fabricaExt varchar(3), productoExt varchar(50))
begin
	declare cuantos int;
	...
	select sum(cantidad)
		FROM Pedidos
        where fabrica = fabricaExt and producto = productoExt ;

	set sql_safe_updates = 0;
	if (cuantos > 3) then
		update productos 
			set precio = precio * 1.05
		where idProducto = productoExt;
	else 
    	update productos 
			set precio = precio * 1.05
		where idProducto = productoExt;
