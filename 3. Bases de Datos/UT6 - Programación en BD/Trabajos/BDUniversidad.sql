...
DELIMITER $$
create procedure actualizaNota(in DNIExt varchar(9),nombreExt varchar(100),
								notaExt decimal(4,2))
begin
	declare vIdP int;
	declare vIdA int;
	...
	if not exists (select *
				from persona
				where dni = DNIExt) then
		select "DNI inválido";
    else
		if not exists (select *
				from asignatura
				where nombre = nombreExt) then
			select "Asignatura inválida";
		else
			select id into vIdP
				from persona
                where dni = DNIExt;
			select id into vIdA
				from asignatura
                where nombre = nombreExt;
			select id into vIdC
				from curso_escolar where anyo_inicio = 2020 and anyo_fin = 2021;
			if not exists (select *
				from alumno_se_matricula_asignatura
				where id_alumno = vIdP and id_asignatura = vIdA and id_curso_escolar = vIdC) then
				select "No existe matrícula";
			else
				update alumno_se_matricula_asignatura
					set nota = notaExt
                    where id_alumno = vIdP and id_asignatura = vIdA and id_curso_escolar = vIdC; 
				select "Se ha actualizado correctamente la calificación";
                select *
					from alumno_se_matricula_asignatura;
			end if;
		end if;
...
call actualizaNota("26902806M","Sistemas Inteligentes",10);
