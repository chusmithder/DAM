...
DELIMITER $$
create procedure P01(nombre varchar(20))
begin
	declare texto varchar(50);
    if nombre = "Carlos" then
		set texto = concat("Bienvenido ",nombre);
	end if;
...
DELIMITER $$
create procedure P02(nombre varchar(20))
begin
	declare texto varchar(50);
    if nombre = "Carlos" then
		set texto = concat("Bienvenido ",nombre);
	else
		set texto = concat(nombre," NO tienes permiso");
	end if;
    select texto as "Salida";
end
$$
...
DELIMITER $$
create procedure P03(nombre varchar(10),
    ...
    if nombre = "Carlos" then
		if contrasena = "12345" then
			if codigo = "7abc" then
				...
				set texto = "Codigo incorrecto";
			end if;
		else
			set texto = "Contraseña incorrecta";
		end if;
	else
		set texto = "Nombre incorrecto";
	end if;
    select texto as "Salida";
end
$$
...
call P03("Carlos","12345","7abc");
*//*
drop procedure if exists P03B;
DELIMITER $$
create procedure P03B(nombreExt varchar(20),apellido1Ext varchar(20),apellido2Ext varchar(20))
begin
	select *
		from Socios
...
$$
call P03B("Itsaso","Martínez","Latasa");*/
/*
drop procedure if exists P04;
DELIMITER $$
create procedure P04(ISBN_Externo varchar(10))
begin
	declare var_ISBN varchar(10);
    set var_ISBN = (select ISBN 
						from Libros
                        where ISBN = ISBN_Externo);
	if var_ISBN is not null then
		select concat("El libro con ISBN: ",ISBN_Externo," existe");
	else	
		select concat("El libro con ISBN: ",ISBN_Externo," no existe");
	end if;
end
$$
DELIMITER ;
...
DELIMITER $$
create procedure P05(ISBN_Externo varchar(10))
begin
	if exists (select ISBN 
					from Libros
					where ISBN = ISBN_Externo) then
		select concat("El libro con ISBN: ",ISBN_Externo," existe");
	else	
		select concat("El libro con ISBN: ",ISBN_Externo," no existe");
	end if;
...
DELIMITER $$
create procedure P06(ISBN_Externo varchar(10))
begin
	if ISBN_Externo in (select ISBN 
					from Libros
					where ISBN = ISBN_Externo) then
		select concat("El libro con ISBN: ",ISBN_Externo," existe");
	else	
...
DELIMITER $$
create procedure P07(apellido2Ext varchar(20))
begin
	declare texto varchar(50);
    declare cuantos int;
    case apellido2Ext 
		when "Etxeberria" then
			...
			set texto = concat(apellido2Ext, " está  ",cuantos," veces");
		when "Gomez" then
			set cuantos = (select count(*) 
								from Socios 
                                where Apellido2 = apellido2Ext);
			...
			set texto = concat(apellido2Ext, " no está  repetido");
	end case;
    select texto as "Salida";
end
$$
