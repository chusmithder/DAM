...
/*
select nom_trabajador, (tarifa_hr * 40) as "t semanal"
	from trabajador
    where oficio = "Electricista"
    order by nom_trabajador;

select t1.nom_trabajador, t1.id_trabajador, t1.id_supv,"-", t2.nom_trabajador, t2.id_trabajador 
	from trabajador t1
    inner join trabajador t2 on t2.id_trabajador = t1.id_supv;
...
select distinct(a.nom_trabajador)
	from trabajador a, asignacion b, edificio e
    where a.id_trabajador = b.id_trabajador and b.id_edificio = e.id_edificio
    and e.tipo = "Oficina"
    order by a.nom_trabajador;

select t.nom_trabajador, count(e.id_edificio) as num_edificios, avg(e.nivel_calidad) as calidad_media
	from trabajador t
    left join asignacion a on a.id_trabajador = t.id_trabajador
	left join edificio e on a.id_edificio = e.id_edificio
    group by t.nom_trabajador
    order by num_edificios, calidad_media desc;
*/
-- con inner join
select distinct t.nom_trabajador, e.tipo
	from trabajador t
    inner join asignacion a on a.id_trabajador = t.id_trabajador
    inner join edificio e on e.id_edificio = a.id_edificio
    where e.tipo = "Oficina"
    order by t.nom_trabajador;
