/*
Actividad evaluable exhaustiva UT6 (Condicionales)	Módulo:GEBD/BADA 	Ciclo: ASIR1/DAM1
...
/*Ejercicio 1*/
drop procedure if exists infoProfesor;
DELIMITER $$
create procedure infoProfesor(pIdPersona int, pIdDepartamento int)
begin
	declare texto varchar(100) default ""; --
    declare varIdDepartamento int; --
    declare varIdPersona int; --
    declare varNombreProfesor varchar(25);
    declare varApellido1Profesor varchar(50);
    declare varNombreDepartamento varchar(50);
    declare varNotaMediaAlumnos decimal(4,2);
    
    set varIdDepartamento = (select id
								from departamento
								where id = pIdDepartamento);
	set varIdPersona = (select id_profesor
							from profesor
							where id_profesor = pIdPersona);
	
    if (varIdDepartamento is not null) then
		if (varIdPersona is not null) then
			if exists (select *
						from profesor
						where id_departamento = pIdDepartamento 
							and id_profesor = pIdPersona) then
					set varNombreProfesor = (select nombre
												from persona
												where id = pIdPersona);
					
                    set varApellido1Profesor = (select apellido1
												from persona
												where id = pIdPersona);
					
                    set varNombreDepartamento = (select nombre
													from departamento
													where id = pIdDepartamento);
                     set varNotaMediaAlumnos = (select avg(nota)
												from alumno_se_matricula_asignatura
												where id_asignatura in (select id
																		from asignatura
																		where id_profesor = 7));
				set texto = concat("Los alumnos del profesor ", varNombreProfesor," ", varApellido1Profesor,
					" del departamento ", varNombreDepartamento, "tienen como nota media ", varNotaMediaAlumnos);
            else 
				set texto = concat("El profesor no está adscrito a ese departamento");
            end if;
        else
			set texto = concat("El identificativo de esa persona no corresponde a ningun profesor");
        end if;
    else
		set texto = concat("El departamento no existe");
    end if;
...
$$
DELIMITER ;
call infoProfesor(11, 10);
call infoProfesor(14, 6);
call infoProfesor(11, 7);
call infoProfesor(2, 2);

/*Ejercicio 2*/
drop procedure if exists infoPersona;
DELIMITER $$
create procedure infoPersona(pIdPersona int)
begin
	declare texto varchar(100) default ""; --
	declare varIdPersona int; --
    declare varIdProfesor int; --
    declare varIdAlumno int; --
	-- alumno
    declare varNotaMaxima decimal(4,2);
    declare varNumAsignaturas int;
    -- profesor
    declare varDepartamento varchar(50);
    declare varNumGradosDistintos int ;
    set varNumGradosDistintos  = 0;
    
    set varIdPersona = (select id
						from persona
                        where id = pIdPersona);
    
    if (varIdPersona is not null) then
		set varIdProfesor = (select id_profesor
						from profesor
                        where id_profesor = pIdPersona);
	
		set varIdAlumno = (select id_alumno
							from alumno_se_matricula_asignatura
							where id_alumno = pIdPersona
							limit 1);
		if (varIdAlumno is not null) then
			set varNotaMaxima = (select max(nota)
						from alumno_se_matricula_asignatura
						where id_alumno = pIdPersona);


			set varNumAsignaturas = (select count(distinct id_asignatura)
										from alumno_se_matricula_asignatura
										where id_alumno = pIdPersona);
		set texto = concat("El alumno ", pIdPersona, " esta matriculado en ",varNumAsignaturas, " asignaturas",
			", y su nota maxima es: ", varNotaMaxima);
		else -- puede pasar que no toda persona sea alumno o profesor
			if (varIdProfesor  is not null) then
				set varDepartamento  = (select d.nombre
						from profesor p
						inner join departamento d on p.id_departamento = d.id
						where p.id_profesor = pIdPersona);

				set varNumGradosDistintos = (select count(distinct id_grado)
												from asignatura
												where id_profesor = pIdPersona);
                set texto = concat("El profesor ", pIdPersona, " pertenece al departamento ", varDepartamento, 
					" e imparte ", varNumGradosDistintos, " grados");
			end if;
        end if;
    else
		set texto = concat("No existe ninguna persona con id ", pIdPersona);
    end if;
    select texto as "Mensaje";
end
$$
