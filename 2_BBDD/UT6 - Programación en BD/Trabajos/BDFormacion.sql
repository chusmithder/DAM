...
DELIMITER $$
create procedure cursosProfesor(NOMPExt varchar(25))
begin
	declare cuantos int;
    declare nombreCompleto varchar(25);
    declare codigo varchar(2);
    declare cuantosCursos int;
	declare Salida text default " ";
    select count(*) into cuantos
		from Profesores
        where NOMP like concat("%", NOMPExt,"%" )
        if cuantos > 1 then
			set Salida = "Hay varios profesores con este nombre\n";
		else 
			if cuantos = 1 then
				select NOMP, CODP into nombreCompleto, codigo
					from Profesores
					where NOMP like concat("%", NOMPExt,"%");
				select *
					from cursillos
                    where CODP = codigo;
				set Salida = concat("El profesor", nombreCompleto, " con codigo ", codigo, 
					" esta dando ". cuantosCursos, " cursillos.");
                    
			else 
            set Salida = "No existe ese profesor \n";
            end if;
	end if;
end
$$
