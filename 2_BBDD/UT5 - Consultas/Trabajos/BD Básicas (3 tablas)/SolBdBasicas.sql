...
-- 1.2 ///////////////////////////////////////////////////////////////////////
select nombre, apellidos, sueldo, sueldo*1.2 as "Subida de sueldo"
	from empleados;
    
select titulo, precio, precio*0.75 as "Precio con dscto"
	from libros;
/*
select destinatario, importe, importe + 2.99 as "Precio con gastos"
	from factura;
*/
...
select *
	from libros 
    where tipo != "divulgación";
    
select * 
	from empleados
    where nombre not in("Luis");

-- 1.3.2
select *
	from empleados
    where sueldo < 2000 and nombre = "David";
    
select *
	from libros
    where precio > 20 or autor = "anónimo";
    
select *
	from facturas
    where not importe < 100;
	
select *
	from empleados
    where telefono > 948000000 and not nombre = "Juan";
    
select *
	from empleados
    where sueldo < 2000 or sueldo > 5000;
...
select *
	from empleados
    where puesto in("operario", "supervisor");

select *
	from facturas
    where importe > 50 and not importe > 200;
    
select *
	from libros
    where precio > 30 or precio < 15;
    
-- 1.3.3 BETWEEN
select *
	from empleados
    where telefono between 948000000 and 980000000;

select *
	from libros
    where (precio between 15 and 30);

select *
	from libros
    where ISBN not between 431501467  and 705940145;
...
select *
	from facturas
    where destinatario like "Alan %";

select *
	from facturas
    where destinatario like "% _eumann";
    
select *
	from libros
    where titulo like "El % de _or_es";
...
select concat(titulo, ": ", precio, "€") as "Lista de Precios"
	from libros;
/*
select concat(Destinatario, ": ", precio, "€") as "Lista de Precios"
	from libros;*/

select lower(nombre) as "Nombre en minuscula"
	from empleados;

-- 12
select length(nombre) as "Tamanio Nombre"
	from empleados;

-- select 
select substring(apellidos, 1 , 2 ) as "Nombre en minuscula"
	from empleados;

select Apellidos, substring(Apellidos, length(Apellidos) - 1, length(Apellidos)) as "Nombre en minuscula"
	from empleados;

-- 1.5.2 Funciones Matematicas
select Nombre, Apellidos,round(Sueldo,1) 
	from empleados;

select titulo, round(Precio) 
	from libros;

select Destinatarop, round(importe,1) 
	from facturas;

-- 1.5.3
select destinatario, dayofmonth(fecha_hora) 
	from facturas;

select destinatario, dayofweek(fecha_hora)
	from facturas;

select destinatario, month(fecha_hora)
	from facturas;

select destinatario, year(fecha_hora) 
	from facturas; -- calendario ingles (empieza por domingo)
    
select destinatario, hour(fecha_hora)
	from facturas;

-- 1.5.3.9
select destinatario, datediff(curdate(), fecha_hora) as "Diferencia fechas"
	from facturas;
...
/*
-- 1.5.3.17
select nombre, timestampdiff(month,fecha_hora, now()) as "Antiguedad"
from facturas;
*/
-- 1.5.4.1
select upper(concat(nombre, " ", apellidos)) as "Nombre Completo"
	from empleados;

-- 1.5.4.2
select concat(upper(substring(nombre, 1, 1)), ", ", Apellidos) as "Nombre Completo"
from empleados
where substring(apellidos, length(apellidos)-4, length(apellidos)) = "Pérez";
