...
-- Subconsultas de comparacion, un resultado
select s.Nombre, l.ISBN, a.FechaAlquiler
	from Socios s
...
-- Subconsultas de pertinencia, varios resultados...
select ISBN, Titulo
	from Libros
    where ISBN in
			(select ISBN
...
/*
select l.ISBN, l.titulo, e.Nejemplar
	from libros l
		join ejemplares e on l.ISBN = e.ISBN
	where e.NEjemplar = 301;
...
-- DATOS DE LOS SOCIOS QUE CUMPLAN que su fnac sea mayor que cualquiera de los que tengan Martinez como 1er apellido
...
-- sacar titulo y autores de los libros cuyo estado sea "NO" EN LA TABLA ejemplares
-- primero subconsultas
select Estado
	from Ejemplares
    where estado = "No";
...
select Titulo, Autor
	...
	where ISBN IN
		(select ISBN
			...
			where estado = "No");
*/
-- SACAR TITULO Y AUTOR DE LOS LIBROS, DE AQUELLOS LIBROS QUE TIENEN AL MENOS 2 "si" en el estado
--  where Estado = "Si" and count(Estado) >= 2;
-- primero subconsulta... (devuelva una sola cosa)
/* debemos borrar count(*)
select ISBN, count(*)
	from Ejemplares
    where Estado = "Si" 
    group by ISBN
