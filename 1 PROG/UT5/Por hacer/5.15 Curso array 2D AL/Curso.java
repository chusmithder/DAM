...
public class Curso
{
     ...
     */
    public Curso()    {
        notas = new int[MAX_ESTUDIANTES][MAX_ASIGNATURAS];//10 X 6
        inicializar(notas);
    }
    ...
    public String toString() {
        String resul = "Notas de los alumnos: \n";
        for (int i = 0; i < notas.length; i++) {
            resul += String.format("%-11s","Alumno " + (i + 1) +" :");
            for (int j = 0; j < notas[j].length; j++) {
                resul += String.format("%4d", notas[i][j]);
            }
            resul += "\n";
        }
    ...
    public double[] calcularMedias()   {
        ...
        for (int col = 0; col < MAX_ASIGNATURAS; col++) {
            double suma = 0.0;
            for (int fila = 0; fila < notas.length; fila++) {
                suma += notas[fila][col];
            }
